<?php
// Database Configuration
// Priority: Environment Variables -> TiDB Cloud (Default)

$servername = getenv('DB_HOST') ?: "gateway01.ap-southeast-1.prod.aws.tidbcloud.com";
$port = getenv('DB_PORT') ?: "4000";
$username = getenv('DB_USER') ?: "3CxVvU9yW5XoNFr.root";
$password = getenv('DB_PASS') ?: "B88J4GhVFxQXC5Cu";
$dbname = getenv('DB_NAME') ?: "ai_speaking_db";

// Initialize MySQLi
$conn = mysqli_init();

// TiDB Cloud requires a secure connection (SSL)
// We enable SSL but don't verify the certificate for simplicity in this context, 
// but in production, you might want to verify the CA.
$conn->ssl_set(NULL, NULL, NULL, NULL, NULL);

// Connect
if (!$conn->real_connect($servername, $username, $password, $dbname, $port, NULL, MYSQLI_CLIENT_SSL)) {
    die("Connection failed: " . mysqli_connect_error());
}

// Set charset
$conn->set_charset("utf8mb4");
?>